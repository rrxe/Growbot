...
export async function broadcastTaskReminder(
  ...
  const text = [
    '⏰ فيه مهام تنتظرك الحين في Stormy!',
    '',
...
async function sendHome(
  ...
  const lines = [
    'مرحبًا بك في بوت تبادل قنوات وبوتات Stormy',
    '',
...
export async function startBot() {
  ...
  bot.command(
    ...
    async (
      ...
      await ctx.reply(
        'افتح تطبيق Stormy من الزر التالي:',
        {
          reply_markup:
            miniAppKeyboard()
        }
    ...
    async (
      ...
      await ctx.reply(
        [
          'شروط استخدام Stormy',
          '',
          '• النقاط داخل Stormy تُستخدم فقط لنشر وتنفيذ المهام على المنصة.',
